...
$mysqli = new mysqli("localhost","root","","doctordb");
...
if(count($errors)==0){
  ...
  $_SESSION['success']="inciaste sesion correctamente";
...
if (isset($_POST['Login'])) {
		...
		$Password 	= $mysqli -> real_escape_string($_POST['password']);
if (empty($UserID)) {
	array_push($errors,"ID de usuario requerida");
	# code...
}
if (empty($Password)) {
  ...
  header('location:../presentaionlayer/patient/index.php');
		...
		array_push($errors,"La ID o la contraseña no son correctas");
...
if (empty($DoctorID2)) {
	array_push($errors,"ID es requerida");
	# code...
}
if (empty($DoctorPassword2)) {
	...
	$_SESSION['DoctorID']=$DoctorID2;
  	...
  	header('location:../presentaionlayer/doctor/index2.php'); 
		...
		array_push($errors,"La ID o la contraseña son incorrectas");
...
if (empty($adminID)) {
	array_push($errors,"ID requerida");
	# code...
}
if (empty($adminpassword)) {
	...
	$_SESSION['AdminID']=$adminID;
  	...
  	header('location:../presentaionlayer/admin/index3.php'); 
		...
		array_push($errors,"ID o contraseñas incorrectas");
